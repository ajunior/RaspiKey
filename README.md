...
* **No impact on keyboard input latency**. Measurements with the Apple Wireless Keyboard on RaspiKey achieve a minimum latency of **12 msec**. This performs as good as the Apple Magic Keyboard does when directly connected to the PC's USB port (that is, no Bluetooth and no RaspiKey in the middle). For a relative comparison and discussion on keyboard latencies you can look [here](https://danluu.com/keyboard-latency/).
