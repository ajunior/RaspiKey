...
DBusMessage* g_dbus_proxy_method_call_blocking(GDBusProxy *proxy, const char *method, GDBusSetupFunction setup, void *user_data, DBusError* error)
{
	struct method_call_data *data;
	GDBusClient *client;
	DBusMessage *msg;
	...
	msg = dbus_message_new_method_call(client->service_name, proxy->obj_path, proxy->interface, method);
	if (msg == NULL)
		return NULL;
	...
	return g_dbus_send_message_with_reply_blocking(client->dbus_conn, msg, METHOD_CALL_TIMEOUT, error);
}
